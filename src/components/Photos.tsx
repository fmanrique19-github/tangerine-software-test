...
interface IFPhotos {
  ...
  toSearch: string;
  displayAlbums(): void;
  displayPhotos1(search: string): void;
}

const Photos = (props: IFPhotos): JSX.Element => {
  const { photos, md, className, toSearch, title } = props;
  const copyPhotos =
    toSearch && toSearch !== ""
      ? photos.filter(find => find.title.includes(toSearch))
      : photos;
  useEffect(() => {
    return () => {
  ...
  const search = (e: React.FormEvent<HTMLInputElement>) => {
    props.displayPhotos1(e.currentTarget.value);
  };
      ...
      {!photos ? (
        ...
        <MDBJumbotron className="photo-mobile">
          ...
          <MDBListGroup>
            <MDBListGroupItem
              href="#"
              className="text-center fixed-title"
              active
              key="0"
            >
              <h3>Album's Photos - {title}</h3>
            </MDBListGroupItem>
